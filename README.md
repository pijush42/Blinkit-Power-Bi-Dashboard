# 🛒 Blinkit Power BI Dashboard

A comprehensive and interactive Power BI dashboard created for **Blinkit – India's Last Minute App**, providing deep insights into product sales, outlet performance, and consumer behavior.

## 📊 Features

- **Total Sales & KPI Cards**: Visual indicators for sales, number of items, and average rating.
- **Dynamic Filters**: Slicers for outlet location type, size, and item type for personalized insights.
- **Visual Breakdown**:
  - Donut and bar charts to illustrate fat content and item types.
  - Line chart showcasing outlet establishment trends over time.
  - Segmentation by outlet size and location.
- **Comparative Metrics**: Breakdown by outlet type with total sales, item counts, average sales, and visibility metrics.

## ⚙️ Tech Stack

- **Power BI**: Dashboard creation and data modeling
- **DAX**: Measures and calculated columns
- **SQL (external)**: Data preprocessing
- **Data Visualization**: Charts, slicers, KPI cards

## 🧠 Use Cases

- Business performance monitoring
- Data storytelling for stakeholders
- Operational optimization by outlet tier and location

## 📷 Preview

![Blinkit](https://github.com/user-attachments/assets/808e4353-7ad3-4e9e-bd91-d4b65af9a1f8)


## 🗂 Files Included



## 💡 Author

**Pijush Kuri**  
📫 pijushkuri42@gmail.com  
🔗 [LinkedIn](https://www.linkedin.com/in/pijush-kuri-uk42) | [GitHub](https://github.com/pijush42)

---

> Feel free to fork, use, or suggest improvements!
